...
const int ARM_EDGE = 350;               // 중립(약512)에서 이만큼 벗어나면 "끝까지 민 것"
...
void armAuto() {
  autoMode = true;
  armPosePrev = false;
  // calibrateJoystickCenter();    // v1.3.1: 512 고정 사용

  // v1.3.1: 진입 시 4축 모두 90도(기본자세)로 정렬
  ...
  autoReady = false;               // 스틱이 중립으로 돌아오기 전까지 제어 보류
  lastAutoActivity = millis();
  ledNotify(3);                    // 시동 알림: 3회 깜빡
}
...
void sendReady() {
  Serial.println(F("READY,CUBELINK,v1.3"));
}
